...
import conftest, generator, types_
from jumanji.testing.pytrees import assert_trees_are_different, assert_trees_are_equal


class TestDummyGenerator:
    @pytest.fixture
    def dummy_generator(self) -> conftest.DummyGenerator:
        return conftest.DummyGenerator()
    
    def test_dummy_generator__properties(self, dummy_generator: conftest.DummyGenerator) -> None:
        """Validate that the dummy instance generator has the correct properties.
        """
        assert dummy_generator.num_nodes == 4
        assert dummy_generator.max_length == 2
        
    def test_dummy_generator__call(self, dummy_generator: conftest.DummyGenerator) -> None:
...
class TestUniformGenerator:
    ...
    def uniform_generator(self) -> generator.UniformGenerator:
        return generator.UniformGenerator(
            num_nodes=20,
            max_length=2,
        )        
