...
class TestSparseOP:
    def test_op_sparse_reset(self, op_sparse_reward: OP) -> None:
        ...
        # Prize of the depot
        assert state.prizes[DEPOT_IDX] == 0.0

        # Check that the state is made of DeviceArrays, this is false for the non-jitted
        # reset function since unpacking random.split returns numpy arrays and not
        # device arrays.
    ...
    def test_op_sparse__trajectory_action(self, op_sparse_reward: OP) -> None:
        """Checks that the agent goes back to the depot when the remaining travel
        budget does not allow visiting extra nodes and that appropriate reward is
        recieved.
        """
...
class TestDenseOP:
    ...
    def test_op_dense_step(self, op_dense_reward: OP) -> None:
        ...
        key = jax.random.PRNGKey(0)
        step_key, reset_key = jax.random.split(key)
        state, timestep = op_dense_reward.reset(reset_key)
    ...
    def test_op_dense__trajectory_action(self, op_dense_reward: OP) -> None:
        ...
        budget does not allow visiting extra nodes and that appropriate reward
        is recieved.
        """
