...
def op_dense_reward(dense_reward: DenseReward) -> OP:
    ...
    """
    return OP(
        generator=ProportionalGenerator(num_nodes=20, max_length=2),
...
class DummyGenerator(Generator):
    ...
    def __call__(self, key: chex.PRNGKey) -> State:
        ...
        # distances = jnp.array([0, 0.07, 0.07, 0.11, 0.15])
        distances = self._compute_distances(coordinates)
        prizes = self._generate_prizes(key, distances[0])

        # The initial travel budget (max length)
        budget = jnp.array(0.4, jnp.int32)
